...
本次版本基于 v0.5，仅进行前端视觉与布局升级，不改动 backend 逻辑、接口与消息协议。

- 进入页改为“登录窗口式”居中卡片：全屏高度居中、白色圆角卡片、轻阴影、结构更清晰。
- 聊天页改为“居中大卡片”布局：页面不再只占上半区，整体居中，层次拆分为顶部信息区 / 在线用户区 / 消息区 / 输入区。
- 统一美化了消息气泡、输入框、按钮与状态标签，整体风格更简约、舒适。
- 保留原有全部功能：多房间、在线用户列表、在线人数、系统提示消息、WebSocket 实时聊天、Enter 发送、自动滚动、退出聊天室。

### v0.5.1 手动验证步骤

1. 启动 backend 与 frontend，访问 `http://localhost:5173`。
2. 观察进入页：是否为全屏居中白色卡片，包含标题、说明、昵称输入、房间选择与“进入聊天室”按钮。
3. 输入昵称并进入任意房间，观察聊天页：整体是否为居中大卡片，且页面高度铺满可视区。
4. 确认顶部信息区能显示当前用户/房间/在线人数，在线用户区可实时更新列表。
5. 发送普通消息、观察系统提示消息，确认消息气泡样式区分“自己/他人/系统”。
6. 在输入框按 Enter 发送，确认可正常发送；消息列表应自动滚动至底部。
7. 切换房间后确认消息与在线状态按房间隔离；点击“退出聊天室”确认可返回进入页。
